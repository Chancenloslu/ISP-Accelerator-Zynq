...
#define CMD_SET_ADDRRD _IO('r', 0)
#define CMD_SET_ADDRWR _IO('r', 1)
#define CMD_SET_START _IOW('r', 2, int)
#define CMD_GET_END   _IOR('r', 3, int)
...
int misc_open(struct inode *inode, struct file *file){
	...
	printk("Hello, rgb2grayscale converter"); 		//info given by printk can not showed through ssh
	
	//cpu_addr_rd = kmalloc(BUF_SIZE, GFP_KERNEL);
	//dma_handler_rd = dma_map_single(dev, cpu_addr_rd, BUF_SIZE, DMA_TO_DEVICE);
	cpu_addr_rd = dma_alloc_coherent(dev, BUF_SIZE, &dma_handler_rd, GFP_KERNEL);
	/*if(dma_mapping_error(dev, dma_handler_rd)){
		printk("dma mapping for readaddress error");
		goto map_error_handling1;
	}*/
	printk("cpu_addr_rd = 0x%p, dma_handler_rd = %lld", cpu_addr_rd, dma_handler_rd);
	
	//cpu_addr_wr = kmalloc(BUF_SIZE, GFP_KERNEL);
	//dma_handler_wr = dma_map_single(dev, cpu_addr_wr, BUF_SIZE, DMA_FROM_DEVICE);
	cpu_addr_wr	= dma_alloc_coherent(dev, BUF_SIZE, &dma_handler_wr, GFP_KERNEL);
	/*if(dma_mapping_error(dev, dma_handler_rd)){
		printk("dma mapping for writeaddress error");
	...
	/*map_error_handling2:
		dma_unmap_single(dev, dma_handler_wr, BUF_SIZE, DMA_FROM_DEVICE);
	map_error_handling1:
		dma_unmap_single(dev, dma_handler_rd, BUF_SIZE, DMA_TO_DEVICE);*/
	ignore_this_device:
		return -1;
}
...
ssize_t misc_write (struct file *file, const char __user *ubuf, size_t size, loff_t *loff_t){
	...
	if(state == WRITE_START_SIG) {
		if(nr == 0) {
			*ADDRESS_RD = dma_handler_rd;
			printk("dma read at %lld", dma_handler_rd);
			nr++;
		}else if(nr == 1) {
			*ADDRESS_WR = dma_handler_wr;
			printk("dma write at %lld", dma_handler_wr);
			nr++;
		}else {
			*START = op;
			printk("start conversion...");
...
static int misc_init(void) {
	...
	if(ret<0) {
		printk("miscdevice >>rgb2gray<< register failed!");
		return -1;
	}
	printk("miscdevice >>rgb2gray<< register succeeded!");
	
	vir_myMisc = ioremap(CNVT_ADDR, RANGE);
	if(vir_myMisc == NULL) {
		printk("rgb2gray ioremap error");
		return -EBUSY;
	}
	printk("rgb2gray ioremap succeeded.");
	return 0;
}
...
static void misc_exit(void) {
	...
	iounmap(vir_myMisc);
	printk("miscdevice >>rgb2gray<< deregister succeeded!");
}
